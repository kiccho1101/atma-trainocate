...
for fold, (train_idx, valid_idx) in enumerate(folds):
    ...
    if "lgbm" in models:
        ...
        y_pred_log_lgb = lgb_model.predict(_valid_df[features])
    ...
    if "cat" in models:
        ...
        y_pred_log_cat = cat_model.predict(_valid_df[features])
    ...
    if models == "lgbm+cat":
        ...
        lr.fit(x, _valid_df["likes_log"])
        y_pred = np.expm1(lr.predict(x))
        y_pred[y_pred < 0] = 0
        rmsle = np.sqrt(mean_squared_log_error(y_true, y_pred))
        print(f"------------------- stacking rmsle {rmsle} -----------------------")
        print(f"------------------- stacking coef {lr.coef_} -----------------------")
        print(
            f"------------------- stacking intercept {lr.intercept_} -----------------------"
        )
        print()
...
for col in ["color_r", "color_g", "color_b"]:
    if col in raw.train.columns:
        raw.train = raw.train.drop(col, axis=1)
_df = raw.train.sort_values("likes", ascending=False).iloc[20:29].copy()
_df = raw.palette.merge(_df, how="inner", on="object_id").sort_values(
    ["likes", "object_id"]
)

fig = plt.figure(figsize=(20, 20))
for i, (index, df) in enumerate(_df.groupby("object_id")):
    ax = fig.add_subplot(3, 3, i + 1)
    left = 0
    for index, row in df.sort_values("ratio", ascending=False).iterrows():
        color = (
            "#"
            + format(row["color_r"], "02x")
            + format(row["color_g"], "02x")
            + format(row["color_b"], "02x")
        )
        ...
        ax.set_title(
            "\n".join(
                [
                    row["title"],
                    "likes={}".format(row["likes"]),
                    "color_h_std={}".format(row["color_h_std"]),
                ]
                + ["color_{}={}".format(c, row[f"color_{c}_mean"]) for c in list("hsv")]
            )
        )
        left += row["ratio"]
