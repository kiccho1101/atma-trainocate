...
] = "/Users/yodaikishimoto/Desktop/kaggle-gpu-2c6aad126765.json"

client = translate.Client()


def translate(t: str):
    if t != t or t is None or t == "":
        return ""
    translated = client.translate(t, source_language="nl", target_language="en")
    time.sleep(0.5)
    return translated
...
# %%
translated = []
for t in tqdm(raw.train["description"], total=len(raw.train)):
    translated.append(translate(t))
raw.train["description_en"] = translated
raw.train.to_csv("train.csv")

# %%

_translated = []
for t in tqdm(raw.test["description"], total=len(raw.test)):
    _translated.append(translate(t))

raw.test["description_en"] = _translated
raw.test.to_csv("test.csv")
...
def pp(des):
    if des != des or des == "":
        return ""
    print(des)
    found = re.findall(r"'translatedText': '([^']*)'", des)
    return found[0]


# train_df = pd.read_csv("train.csv")
# train_df["description_en"] = train_df["description_en"].map(pp)
raw.test["description_en"] = raw.test["description_en"].map(lambda r: r["translatedText"] if r == r and r != "" else "")
