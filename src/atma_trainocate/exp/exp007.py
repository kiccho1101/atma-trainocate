...
def fe_long_title_pca(raw: RawData, n_components: int) -> RawData:
    train_long_title_encoded = np.load(
        Config.root_dir / f"data/train_long_title_encoded{Config.bert_model}.npy"
    )
    test_long_title_encoded = np.load(
        Config.root_dir / f"data/test_long_title_encoded{Config.bert_model}.npy"
    )
...
def fe_description_en_pca(raw: RawData, n_components: int) -> RawData:
    train_description_en_encoded = np.load(
        Config.root_dir / f"data/train_description_en_encoded{Config.bert_model}.npy"
    )
    test_description_en_encoded = np.load(
        Config.root_dir / f"data/test_description_en_encoded{Config.bert_model}.npy"
    )
    concated = np.concatenate(
        (train_description_en_encoded, test_description_en_encoded)
    )
    pca = PCA(n_components=n_components).fit(concated)
    train_des_pca = pca.transform(train_description_en_encoded)
    test_des_pca = pca.transform(test_description_en_encoded)
    raw.train.loc[
        :, [f"description_en_pca_{i}" for i in range(n_components)]
    ] = train_des_pca
    raw.test.loc[
        :, [f"description_en_pca_{i}" for i in range(n_components)]
    ] = test_des_pca
...
def fe(
    raw: RawData, long_title_n_components: int, desc_en_n_components: int
) -> RawData:
...
raw = fe(raw, Config.long_title_n_components, Config.desc_en_n_components)
...
cat_model = CatBoostRegressor(**Config.cat_params, iterations=1300)
cat_model.fit(
...
raw.sample_submission.to_csv(Path.cwd() / "output" / "exp006_2.csv", index=False)
